...
estimate_model <- function(x, t, x_exp1=NULL, t_exp1=NULL, x_exp0=NULL){
  ...
  try(while (((diff.par > 10^-3) || (diff.k > 10^-3)) && (iter <= 80)){
    if (iter == 1){
      ...
      if (is.null(x_exp1) == FALSE & is.null(x_exp0) == FALSE){
        Log.Like.par <- function (lambda,alpha,beta){
          -sum(log((1-p.new)*dpois(x,lambda) + p.new*dnbinom(x,0.5,mu = exp(alpha + beta*t)))) -
                 sum(log(dnbinom(x_exp, 0.5, mu = exp(alpha + beta*t_exp)))) - sum(log(dpois(x_exp, lambda)))
        }
      ...
      } else if (is.null(x_exp1) == FALSE & is.null(x_exp0) == TRUE) { # experimental data on non shooters
        Log.Like.par <- function (lambda,alpha,beta){
          -sum(log((1-p.new)*dpois(x,lambda) + p.new*dnbinom(x,0.5,mu = exp(alpha + beta*t)))) -
      ...
      if (is.null(x_exp1) == FALSE & is.null(x_exp0) == FALSE){
        Log.Like.par <- function (lambda,alpha,beta){
          -sum(log((1-p.new)*dpois(x,lambda) + p.new*dnbinom(x,best.k,mu = exp(alpha + beta*t)))) -
            sum(log(dnbinom(x_exp, best.k, mu = exp(alpha + beta*t_exp)))) - sum(log(dpois(x_exp, lambda)))
        }
      } else if (is.null(x_exp1) == TRUE & is.null(x_exp0) == FALSE) { # experimental data on non shooters
        Log.Like.par <- function (lambda,alpha,beta){
          -sum(log((1-p.new)*dpois(x,lambda) + p.new*dnbinom(x,best.k,mu = exp(alpha + beta*t)))) -
