...
Log.Like <- function (x,t,k,p,lambda,alpha,beta){
...
## the function take (k,p,lambda,alpha,beta) as input parameters
Log.Like.1 <- function (theta, x_vec, t_vec){
  sum(log((1-theta[1])*dpois(x_vec,theta[2]) + theta[1]*dnbinom(x_vec,theta[3],mu = exp(theta[4] + theta[5]*t_vec))))
}
...
estimate_model <- function(x, t, x_exp1=NULL, t_exp1=NULL, x_exp0=NULL,
  ...
  try(while (((diff.par > 10^-3) || (diff.k > 10^-3)) && (iter <= max_iter)){
    if (iter == 1){
      ...
      est.param <- mle2(minuslogl = Log.Like.par, start = list(lambda = par.start[2], alpha = par.start[3], beta= par.start[4]),
    ...
    if (iter == 1){
      est.k <- optim(par = k.start, fn = Log.Like,x=x, t=t,  p = p.new, lambda = cur.param[1],
                     alpha = cur.param[2], beta = cur.param[3], method = 'Brent', lower = 0.1, upper = 100)
    }else{
      est.k <- optim(par = k.vector[iter-1], fn = Log.Like,x=x, t=t, p = p.new, lambda = cur.param[1],
                     alpha = cur.param[2], beta = cur.param[3], method = 'Brent', lower = 0.1, upper = 100)
    }
    p.opt <- c(p.opt, p.new)
    best.k <- est.k$par
  ...
  inf_obs <- -solve(hessian(func = Log.Like.1, x = c(p.new, cur.param[1], best.k, cur.param[2], cur.param[3]), x_vec = x, t_vec = t))
